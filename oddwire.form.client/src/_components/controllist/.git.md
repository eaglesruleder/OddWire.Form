...
| `hidden` | Definition config | Skips rendering when true (gated per leaf / `ControlBase`) |
...
|---|---|---|---|
| Control props | Fill / toggle / select a control | Value reflected live in the control | Memory-only — edits lost on refresh; duplicate params share one value |
