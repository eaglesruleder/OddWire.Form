...
A library of stateless leaf form controls that render a label+field row through a shared `ControlBase` and report edits through a common `onChange(value, key, subkey?)` contract (each leaf passes its own `param` as the key).
...
```
Receive props → ControlBase lays out label + field → user edits field → onChange(value, props.param) → parent owns the value
```

Inputs: **CoreControlProps** (param, label, value, hidden, stacked, onChange) + per-control extras
Output: **onChange(value, key, subkey?)** — controls are stateless; the parent holds the value; leaves supply `props.param` as the key.
