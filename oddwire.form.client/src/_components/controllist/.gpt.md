...
### ControlItem
Props inherit `ControlDef` (+ `onChange`). `switch(type)` spreads `{...props}` into the matching leaf; `default` → bold dark-red `ControlBase label="Error"` box (`className="error" labelClassName="bold"`) reading `Unknown control type: {type}`.
